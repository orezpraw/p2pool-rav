...
class MultiScaleRRDGraph(object):
    def sanitizeName(key):
        key = re.sub(r'%$', r'_Per', key) # Replace percent signs
        key = re.sub(r'\W', r'_', key) # Replace symbols and whitespace
        return key
    
    def __init__(self, name, datadir_path, mType='GUAGE', heartbeat=None, lowerBound=0, upperBound='U', resolution=720, timestep=5):
        self.dataDir = datadir_path;
        self.timestep = timestep # in seconds
        self.heartbeat = heartbeat or ((60*60)//resolution)
        self.name = sanitizeName(name)
        self.fileName = os.join.path(self.dataDir, self.name + '.rrd')        
            
        self.periodConfigs = [
            ('last_hour', 
                'RRA:AVERAGE:0.5:%i:%i' % (ceil((60*60)/(timestep*resolution)), self.resolution),
                'AVERAGE:start=end-1h'), # 1h
            ('last_day', 
                'RRA:AVERAGE:0.5:%i:%i' % (ceil((60*60*24)/(timestep*resolution)), self.resolution),
                'AVERAGE:start=end-24h'), # 1d, we specify the time in hours because days aren't always 24 hours long...
            ('last_week', 
                'RRA:AVERAGE:0.5:%i:%i' % (ceil((60*60*24*7)/(timestep*resolution)), self.resolution),
                'AVERAGE:start=end-168h'), # 7d
            ('last_month', 
                'RRA:AVERAGE:0.5:%i:%i' % (ceil((60*60*24*31)/(timestep*resolution)), self.resolution),
                'AVERAGE:start=end-744h'), # 31d
            ('last_year', 
                'RRA:AVERAGE:0.5:%i:%i' % (ceil((60*60*24*366)/(timestep*resolution)), self.resolution),
        ...
        if not os.path.exists(self.fileName):
            ...
            data_sources = ['DS:%s:%s:%s:%s' % (self.name, heartbeat, str(lowerBound), str(upperBound))]
